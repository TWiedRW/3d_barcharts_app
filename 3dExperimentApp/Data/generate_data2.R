...
pr1 <- generate_data(value1 = 75,
              value2 = 35,
              type = c('Type1'),
              random_process = 'beta',
              save = F) %>% 
  mutate(IDchr = ifelse(Identifier == 'Value1' & !is.na(Identifier), '▲', 
                        ifelse(Identifier == 'Value2' & !is.na(Identifier), '●', '')))

pr2 <- generate_data(value1 = 40,
                     value2 = 50,
                     type = c('Type3'),
                     random_process = 'beta',
                     save = F) %>% 
...
prac = bind_rows(pr1, pr2, pr3, .id = 'practiceID')

write.csv(prac, 'Data/practiceData.csv', row.names = F)
